...
func main() {
	...
	participantsContent, err := ioutil.ReadFile(filepath.Join(dirPath, "participants.txt"))
	if err != nil {
	...
	lastWeekContent, err := ioutil.ReadFile(filepath.Join(dirPath, "last_week.txt"))
	if err != nil {
	...
	err = ioutil.WriteFile(filepath.Join(dirPath, "last_week.txt"), []byte(strings.Join(lastWeek, "\n")), 0644)
	if err != nil {
